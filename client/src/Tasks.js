...
`function capitalizeEachWord(sentence)  {
  ...
  {
    id: 11,
    description: "Напиши функцию addDashes, которая принимает на вход число, а возвращает строку, содержащую переданное число. При этом, если в строке две цифры подряд являются чётными, то между ними необходимо вставить дефис. Например, для числа 205468 результат должен быть \"2-054-6-8\".",
    preCode:
`function addDashes (value) {
    // Напиши тело функции здесь.
}`,
    test:
      `describe("addDashes", function() {
              it('при value=205468, результат должен быть равен "2-054-6-8"' , function() {
                  assert.equal(addDashes(205468), "2-054-6-8");
              });

              it('при value=223344, результат должен быть равен "2-2334-4"' , function() {
                  assert.equal(addDashes(223344), "2-2334-4");
              });

              it('при value=12345, результат должен быть равен "12345"' , function() {
                  assert.equal(addDashes(12345), "12345");
              });
          });`,
    testsCount: 3,
  },
